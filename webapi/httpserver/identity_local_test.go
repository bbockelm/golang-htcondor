...
func TestLocalIdentityResolvesSubjectToAccount(t *testing.T) {
	li := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, nil, writePasswd(t), time.Minute, false, testLogger(t))
...
func TestLocalIdentityFailsClosedWhenTheDatabaseIsUnreadable(t *testing.T) {
	li := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, nil, filepath.Join(t.TempDir(), "does-not-exist"), time.Minute, false, testLogger(t))
...
func TestWarmUpSurvivesAnUnreadableDatabase(t *testing.T) {
	li := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, nil, filepath.Join(t.TempDir(), "nope"), time.Minute, false, testLogger(t))
	li.warmUp(t.Context())
...
func TestLocalIdentityGecosThenUsername(t *testing.T) {
	li := newLocalIdentity(
		[]idmap.Strategy{idmap.StrategyGecos, idmap.StrategyUsername}, nil,
		writePasswd(t), time.Minute, false, testLogger(t))
...
func TestLocalIdentityHalvesAreIndependent(t *testing.T) {
	...
	if li := newLocalIdentity(nil, nil, passwd, time.Minute, false, logger); li != nil {
		t.Error("neither half configured should yield no mapper at all")
	}

	// Subject mapping only: groups stay whatever the token said.
	mapOnly := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, nil, passwd, time.Minute, false, logger)
	...
	groupsOnly := newLocalIdentity(nil, []string{"system"}, passwd, time.Minute, false, logger)
...
func TestSystemGroupsNeverFallBackToTheToken(t *testing.T) {
	li := newLocalIdentity(nil, []string{"system"}, writePasswd(t), time.Minute, false, testLogger(t))
...
func TestAssertedIdentitiesFromOtherPathsAreMappedToo(t *testing.T) {
	h := &Handler{
		localIdentity: newLocalIdentity(
			[]idmap.Strategy{idmap.StrategyGecos}, nil,
			writePasswd(t), time.Minute, false, testLogger(t)),
	}
...
func TestGroupsAreReadForTheMappedAccountOnly(t *testing.T) {
	...
	li := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, []string{"system"},
		writePasswd(t), time.Minute, false, testLogger(t))
