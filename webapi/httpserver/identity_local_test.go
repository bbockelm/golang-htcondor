...
func TestLocalIdentityResolvesSubjectToAccount(t *testing.T) {
	li := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, false, writePasswd(t), time.Minute, false, testLogger(t))
...
func TestLocalIdentityFailsClosedWhenTheDatabaseIsUnreadable(t *testing.T) {
	li := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, false, filepath.Join(t.TempDir(), "does-not-exist"), time.Minute, false, testLogger(t))
...
func TestWarmUpSurvivesAnUnreadableDatabase(t *testing.T) {
	li := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, false, filepath.Join(t.TempDir(), "nope"), time.Minute, false, testLogger(t))
	li.warmUp(t.Context())
...
func TestLocalIdentityHalvesAreIndependent(t *testing.T) {
	...
	if li := newLocalIdentity(nil, false, passwd, time.Minute, false, logger); li != nil {
		t.Error("neither half configured should yield no mapper at all")
	}

	// Subject mapping only: groups stay whatever the token said.
	mapOnly := newLocalIdentity([]idmap.Strategy{idmap.StrategyGecos}, false, passwd, time.Minute, false, logger)
	...
	groupsOnly := newLocalIdentity(nil, true, passwd, time.Minute, false, logger)
...
func TestSystemGroupsNeverFallBackToTheToken(t *testing.T) {
	li := newLocalIdentity(nil, true, writePasswd(t), time.Minute, false, testLogger(t))
