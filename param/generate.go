...
func generate() error {
	...
	// param/SOURCE_VERSION records the upstream HTCondor tag that
	// param_info.in was copied from, so the snapshot's provenance is
	// checkable rather than folklore.
	source, err := readSourceVersion("../param/SOURCE_VERSION")
	if err != nil {
		return err
	}
	...
	fmt.Fprintln(out, "")
	fmt.Fprintln(out, "// ParamInfoSource is the upstream HTCondor release tag that the vendored")
	fmt.Fprintln(out, "// param_info.in snapshot was generated from.")
	fmt.Fprintf(out, "const ParamInfoSource = %q\n", source)
	fmt.Fprintln(out, "")
	...
	fmt.Printf("Generated param_defaults.go with %d parameters from HTCondor %s\n", len(params), source)
	return nil
}

func readSourceVersion(filename string) (string, error) {
	data, err := os.ReadFile(filename)
	if err != nil {
		return "", fmt.Errorf("failed to read SOURCE_VERSION: %w", err)
	}
	v := strings.TrimSpace(string(data))
	if v == "" {
		return "", fmt.Errorf("SOURCE_VERSION (%s) is empty", filename)
	}
	return v, nil
}
